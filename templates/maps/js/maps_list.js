...
var columns = [ checkboxSelector.getColumnDefinition(),
  { id: "id", name: "ID", field: "id", resizable: false, width: 50, sortable: true, sorter: sorterNumeric,
      formatter: linkFormatter = function ( row, cell, value, columnDef, dataContext ) {
            return '<a href="javascript:void(0)" onclick="mapViewer('+dataContext['id']+')">' + value + '</a>';
      }
  },
  ...
  { id: "layers", name: "Layers", field: "layers", resizable: true, width: 200, sortable: true, 
     formatter: layersListDropDown = function ( row, cell, value, columnDef, dataContext ) {
            console.log(value)
            var dropDown = '';
            for (var i = 0; i < value.length; i++) { dropDown += '<option value="'+value[i].layer_id+'">'+value[i].layer_name+'</option>'};
  ...
  { id: "in_use", name: "In use", field: "in_use", resizable: true, width: 50, sortable: true }
...
;
  ...
  $(function () {
    ...
    $.ajax({
          type: "GET",
          url: "{% url 'trees:listMapsLoader' folder.pk %}",
          success: function(r) {
