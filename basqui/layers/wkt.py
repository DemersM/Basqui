...
def dumps(obj, dim=2, decimals=5):
    """
...
def _dump_linestring(obj, fmt, dim):
    """
...
"""
    coords = obj['coordinates']
    ls = 'MULTILINESTRING ((%s))'
    ls %= ', '.join(' '.join(fmt % c for c in pt[:dim]) for pt in coords)
    return ls


def _dump_polygon(obj, fmt, dim):
    """
...
"""
    coords = obj['coordinates']
    poly = 'MULTIPOLYGON ((%s))'
    rings = (', '.join(' '.join(fmt % c for c in pt[:dim]) for pt in ring)
             for ring in coords)
...
def _dump_multipoint(obj, fmt, dim):
    """
...
"""
    coords = obj['coordinates']
    mp = 'MULTIPOINT (%s)'
    points = (' '.join(fmt % c for c in pt[:dim]) for pt in coords)
...
def _dump_multilinestring(obj, fmt, dim):
    """
...
"""
    ...
    linestrs = ('(%s)' % ', '.join(' '.join(fmt % c for c in pt[:dim])
                for pt in linestr) for linestr in coords)
    mlls %= ', '.join(ls for ls in linestrs)
    return mlls


def _dump_multipolygon(obj, fmt, dim):
    """
...
def _dump_geometrycollection(obj, fmt, dim):
    """
...
"""
    ...
    for geom in geoms:
        geom_type = geom['type']
        geoms_wkt.append(_dumps_registry.get(geom_type)(geom, fmt, dim=2))
    gc %= ','.join(geoms_wkt)
    return gc
